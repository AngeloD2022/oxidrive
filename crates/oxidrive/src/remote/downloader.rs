...
use crate::core::models::{Application, Package};
use crate::core::utils;
use crate::remote::products::{ProductsClient, configure_headers};
...
impl<'a> ApplicationDownloader<'a> {
    ...
    async fn exec_download_for_pkg<P: ProgressSink + 'static>(
        ...
        let pkg = Arc::new(pkg.clone()); // or keep &PIMXPackage and clone fields you need
                ...
                loop {
                    ...
                    match resp {
                        ...
                        Ok(resp) => {
                            ...
                            progress.on_range_done(&file_name, buffer.len()).await;
                            f.flush().await?;
    ...
    fn filter_pkgs_for_config(
        ...
        vars.insert(
            "OSVersion".to_owned(),
            EvalValue::Version(self.download_cfg.os_version.clone()),
        );
        vars.insert(
            "installLanguage".to_owned(),
            EvalValue::String(self.download_cfg.locale.clone()),
        );
...
mod tests {
    ...
    use crate::remote::downloader::{ApplicationDownloader, DownloadConfiguration, ProgressSink};
    ...
    async fn test_application_downloader() {
        ...
        let pc = ProductsClient::new(platform).await.unwrap();
        ...
        let product = channel.index.get_latest("PHSP", MacUniversal).unwrap();
