use crate::core::models::{Application, Products};
use crate::core::platform::ProductPlatform;
use crate::remote::index::ChannelReduced;
...
impl ProductsClient {
    ...
    pub async fn get_download_dependencies(
        ...
        let allowed_platforms = self.platform.allowed_platforms();
        ...
        if let Some(dependencies) = &application.dependencies {
            ...
            for dep in &dependencies.dependency {
                let product = if let Some(product) = reduced_ccm.index.get_optimal(
                    &dep.sap_code,
                    &dep.base_version,
                    &allowed_platforms,
                ) {
                    Some(product)
                } else if let Some(product) = reduced_sti.index.get_optimal(
                    &dep.sap_code,
                    &dep.base_version,
                    &allowed_platforms,
                ) {
...
mod tests {
    use crate::core::platform::ProductPlatform;
    use crate::remote::products::{ProductsClient, get_products};
    ...
    async fn test_products_client() {
        ...
        let latest = ch.index.get_latest("PHSP", plat).unwrap();
